...
ACTFSession::ACTFSession(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	if (!HasAnyFlags(RF_ClassDefaultObject))
	{
		OnCreateSessionCompleteDelegate = FOnCreateSessionCompleteDelegate::CreateUObject(this, &ACTFSession::OnCreateSessionComplete);
		OnDestroySessionCompleteDelegate = FOnDestroySessionCompleteDelegate::CreateUObject(this, &ACTFSession::OnDestroySessionComplete);

		OnFindSessionsCompleteDelegate = FOnFindSessionsCompleteDelegate::CreateUObject(this, &ACTFSession::OnFindSessionsComplete);
		OnJoinSessionCompleteDelegate = FOnJoinSessionCompleteDelegate::CreateUObject(this, &ACTFSession::OnJoinSessionComplete);

		OnStartSessionCompleteDelegate = FOnStartSessionCompleteDelegate::CreateUObject(this, &ACTFSession::OnStartOnlineGameComplete);
	}
}

void ACTFSession::OnStartOnlineGameComplete(FName SessionName, bool bWasSuccessful)
{
	...
	if (bWasSuccessful)
	{
		...
		{
			ACaptureTheFlagController* PC = Cast<ACaptureTheFlagController>(*It);
...
void ACTFSession::HandleMatchHasStarted()
{
...
void ACTFSession::HandleMatchHasEnded()
{
	// start online game locally and wait for completion
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
...
EOnlineAsyncTaskState::Type ACTFSession::GetSearchResultStatus(int32& SearchResultIdx, int32& NumSearchResults)
{
	SearchResultIdx = 0;
	NumSearchResults = 0;
...
*/
const TArray<FOnlineSessionSearchResult> & ACTFSession::GetSearchResults() const
{
...
*/
void ACTFSession::OnCreateSessionComplete(FName SessionName, bool bWasSuccessful)
{
...
* @param bWasSuccessful true if the async action completed without error, false if there was an error
*/
void ACTFSession::OnDestroySessionComplete(FName SessionName, bool bWasSuccessful)
{
...
bool ACTFSession::HostSession(TSharedPtr<const FUniqueNetId> UserId, FName SessionName, const FString& GameType, const FString& MapName, bool bIsLAN, bool bIsPresence, int32 MaxNumPlayers)
{
...
void ACTFSession::OnFindSessionsComplete(bool bWasSuccessful)
{
...
void ACTFSession::ResetBestSessionVars()
{
	CurrentSessionParams.BestSessionIdx = -1;
}

void ACTFSession::ChooseBestSession()
{
...
void ACTFSession::StartMatchmaking()
{
	ResetBestSessionVars();
	ContinueMatchmaking();
}

void ACTFSession::ContinueMatchmaking()
{
	ChooseBestSession();
...
void ACTFSession::OnNoMatchesAvailable()
{
	UE_LOG(LogOnlineGame, Verbose, TEXT("Matchmaking complete, no sessions available."));
	SearchSettings = NULL;
}

void ACTFSession::FindSessions(TSharedPtr<const FUniqueNetId> UserId, FName SessionName, bool bIsLAN, bool bIsPresence)
{
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
	{
...
bool ACTFSession::JoinSession(TSharedPtr<const FUniqueNetId> UserId, FName SessionName, int32 SessionIndexInSearchResults)
{
	bool bResult = false;
...
bool ACTFSession::JoinSession(TSharedPtr<const FUniqueNetId> UserId, FName SessionName, const FOnlineSessionSearchResult& SearchResult)
{
	bool bResult = false;
...
* @param bWasSuccessful true if the async action completed without error, false if there was an error
*/
void ACTFSession::OnJoinSessionComplete(FName SessionName, EOnJoinSessionCompleteResult::Type Result)
{
...
bool ACTFSession::TravelToSession(int32 ControllerId, FName SessionName)
{
	IOnlineSubsystem* OnlineSub = IOnlineSubsystem::Get();
	if (OnlineSub)
	{
